...
int main(){
  ...
  TFile *fout = new TFile("../output_L1L2/ub/cutPlots.root","RECREATE");

  //cuts listed in order
  TCut cut1 = "eleClY*posClY<0&&((eleHasL1&&!posHasL1&&abs(posTrkExtrpYL1)<1.5)||(posHasL1&&!eleHasL1&&abs(eleTrkExtrpYL1)<1.5))&&eleHasL2&&posHasL2&&uncP>0.845&&isPair1";
  ...
  tt->Draw("uncM-0.15E-3*(elePX/eleP-posPX/posP)*uncVZ/uncM>>h_m14",cut1 && cut2 && cut3 && cut4 && cut5 && cut6 && cut7 && cut8 && cut9 && cut10 && cut11 && cut12 &&cut13 && cut14);
  ...
  tt->Draw("uncVY:uncM-0.15E-3*(elePX/eleP-posPX/posP)*uncVZ/uncM>>h_yvm",cut1 && cut2 && cut3 && cut4 && cut5 && cut6 && cut7 && cut8 && cut9 && cut10 && cut11 && cut12 && cut13&& cut14);
  
  //plot vx:mass
  TH2F *h_xvm = new TH2F("h_xvm","x vtx vs mass",300,0,0.1,300,-3,3);
  tt->Draw("uncVX:uncM-0.15E-3*(elePX/eleP-posPX/posP)*uncVZ/uncM>>h_xvm",cut1 && cut2 && cut3 && cut4 && cut5 && cut6 && cut7 && cut8 && cut9 && cut10 && cut11 && cut12 && cut13&& cut14);

  //plot vz:mass
  TH2F *h_zvm = new TH2F("h_zvm","z vtx vs mass",400,0,0.1,400,-100,100);
  tt->Draw("uncVZ:uncM-0.15E-3*(elePX/eleP-posPX/posP)*uncVZ/uncM>>h_zvm",cut1 && cut2 && cut3 && cut4 && cut5 && cut6 && cut7 && cut8 && cut9 && cut10 && cut11 && cut12 && cut13&& cut14);
  ...
  ltime->Draw();
  ctime->Update();
  ctime->SaveAs("../output_L1L2/ub/tdiff_cuts.C");
  ctime->Close();
  ...
  lzvtx->Draw();
  czvtx->Update();
  czvtx->SaveAs("../output_L1L2/ub/zvtx_cuts.C");
  czvtx->Close();
  ...
  lmass->Draw();
  cmass->Update();
  cmass->SaveAs("../output_L1L2/ub/mass_cuts.C");
  cmass->Close();
 ...
 h_zvm->Draw("colz");
 vertex->Update();
 vertex->SaveAs("../output_L1L2/ub/vertex_cuts.C");
 vertex->Close();
 ...
 llz->Draw();
 ccz->Update();
 ccz->SaveAs("../output_L1L2/ub/zVertexCutEffects.C");
 ccz->Close();

 

 //Plot the change in number of events from cuts in the peak versus the background
 std::fstream myfile("../output_L1L2/ub/cutEffects.txt", std::ios_base::out);
 ...
 eff->Draw("ap");
 ccr->Update();
 ccr->SaveAs("../output_L1L2/ub/cutEfficiencies.C");
 ccr->Close();
 ...
 llzs->Draw();
 cczs->Update();
 cczs->SaveAs("../output_L1L2/ub/zVertexAccidentals.C");
 cczs->Close();
