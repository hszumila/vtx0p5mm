...
int main(){
  ...
  TFile *f = new TFile("../data/hps_L1L1_blind_10Feb17.root");
  TTree *tt=(TTree*)f->Get("ntuple");
  TFile *fout = new TFile("../output_L1L1/cutPlots.root","RECREATE");

  //cuts listed in order
  TCut cut1 = "eleHasL1&&posHasL1&&eleHasL2&&posHasL2&&uncP>0.845&&isPair1";
  ...
  tt->Draw("uncVY:uncM>>h_yvm",cut1 && cut2 && cut3 && cut4 && cut5 && cut6 && cut7 && cut8 && cut9 && cut10 && cut11 && cut12 && cut13);
  
  //plot vx:mass
  TH2F *h_xvm = new TH2F("h_xvm","x vtx vs mass",300,0,0.1,300,-3,3);
  tt->Draw("uncVX:uncM>>h_xvm",cut1 && cut2 && cut3 && cut4 && cut5 && cut6 && cut7 && cut8 && cut9 && cut10 && cut11 && cut12 && cut13);
  ...
  h_tdiff12->Draw("same");
  h_tdiff13->Draw("same");
  ltime->AddEntry("h_tdiff1","L1L1","l");
  ...
  ltime->Draw();
  ctime->Update();
  ctime->SaveAs("../output_L1L1/tdiff_cuts.C");
  ctime->Close();
  ...
  lzvtx->Draw();
  czvtx->Update();
  czvtx->SaveAs("../output_L1L1/zvtx_cuts.C");
  czvtx->Close();
  ...
  lmass->Draw();
  cmass->Update();
  cmass->SaveAs("../output_L1L1/mass_cuts.C");
  cmass->Close();
 ...
 h_zvm->Draw("colz");
 vertex->Update();
 vertex->SaveAs("../output_L1L1/vertex_cuts.C");
 vertex->Close();
 ...
 llz->Draw();
 ccz->Update();
 ccz->SaveAs("../output_L1L1/zVertexCutEffects.C");
 ccz->Close();

 

 //Plot the change in number of events from cuts in the peak versus the background
 std::fstream myfile("../output_L1L1/cutEffects.txt", std::ios_base::out);
 ...
 eff->Draw("ap");
 ccr->Update();
 ccr->SaveAs("../output_L1L1/cutEfficiencies.C");
 ccr->Close();
 ...
 llzs->Draw();
 cczs->Update();
 cczs->SaveAs("../output_L1L1/zVertexAccidentals.C");
 cczs->Close();
