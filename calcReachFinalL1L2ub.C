...
int main(){
  ...
  TFile *f = new TFile("../output_L1L2/ub/cutPlots.root");
  TFile *f1 = new TFile("../finalZvMub.root");
  TFile *fout = new TFile("../output_L1L2/ub/outputFits.root","RECREATE");
  gROOT->SetBatch(true);
  ...
  canvas->SetLogy();
  std::string pdf_file_name = "../output_L1L2/ub/vertexSliceFits.pdf";
  ...
  for (int ii=0; ii<nBinsT; ii++){
    ...
    zCut[ii] = fitZcutUB(centermass[ii]);
  ...
  zzcut->Draw("alp");
  ccz->Update();
  ccz->SaveAs("../output_L1L2/ub/zcut.C");
  ccz->Close();
  ...
  gxS->Draw("lsame");
  kk1->Update();
  kk1->SaveAs("../output_L1L2/ub/zVm_zCut.C");
  kk1->Close();
  ...
  gx2->Draw("");
  kk2->Update();
  kk2->SaveAs("../output_L1L2/ub/par3_zCut.C");
  kk2->Close();
  ...
  gx3->Draw("");
  kk3->Update();
  kk3->SaveAs("../output_L1L2/ub/par4_zCut.C");
  kk3->Close();

   TCanvas *kk4 = new TCanvas("kk4","z sigma for mass",800,800);
  kk4->cd();
  TGraph *gx4 = new TGraph(nBinsT,centermass,par2);
  gx3->Draw("");
  kk4->Update();
  kk4->SaveAs("../output_L1L2/ub/zSigma.C");
  kk4->Close();
  ...
  cc->Update();
  cc->SaveAs("../output_L1L2/ub/reach.C");
  cc->Close();  
