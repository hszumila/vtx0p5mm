...
int main(){
  ...
  TFile *f = new TFile("../data/ub_0p5cut_L1L1.root");
  TTree *tt=(TTree*)f->Get("ntuple");
  TFile *fout = new TFile("../output_L1L1/ubTest/cutPlots.root","RECREATE");

  //cuts listed in order
  TCut cut1 = "eleHasL1&&posHasL1&&eleHasL2&&posHasL2&&uncP>0.845&&isPair1&&eleClY*posClY<0";
  TCut cut2 = "max(eleTrkChisq,posTrkChisq)<30&&eleTrkChisq+posTrkChisq<30";
  ...
  TCut cut7 = "uncP<1.15*1.056&&(pow((elePhiKink2*-0.98-eleLambdaKink2*0.2)/0.003,2)+pow((eleLambdaKink2*-0.98+elePhiKink2*0.2)/0.0065,2))<1&&(pow((posPhiKink2*-0.98-posLambdaKink2*0.2)/0.003,2)+pow((posLambdaKink2*-0.98+posPhiKink2*0.2)/0.0065,2))<1&&(pow((elePhiKink3*0.98-eleLambdaKink3*0.2)/0.003,2)+pow((eleLambdaKink3*0.98+elePhiKink3*0.2)/0.0065,2))<1&&(pow((posPhiKink3*0.98-posLambdaKink3*0.2)/0.003,2)+pow((posLambdaKink3*0.98+posPhiKink3*0.2)/0.0065,2))<1";
  TCut cut8 = "max(eleMatchChisq,posMatchChisq)<10&&eleMatchChisq+posMatchChisq<10";
  ...
  ltime->Draw();
  ctime->Update();
  ctime->SaveAs("../output_L1L1/ubTest/tdiff_cuts.C");
  ctime->Close();
  ...
  lzvtx->Draw();
  czvtx->Update();
  czvtx->SaveAs("../output_L1L1/ubTest/zvtx_cuts.C");
  czvtx->Close();
  ...
  lmass->Draw();
  cmass->Update();
  cmass->SaveAs("../output_L1L1/ubTest/mass_cuts.C");
  cmass->Close();
 ...
 h_zvm->Draw("colz");
 vertex->Update();
 vertex->SaveAs("../output_L1L1/ubTest/vertex_cuts.C");
 vertex->Close();
 ...
 llz->Draw();
 ccz->Update();
 ccz->SaveAs("../output_L1L1/ubTest/zVertexCutEffects.C");
 ccz->Close();

 

 //Plot the change in number of events from cuts in the peak versus the background
 std::fstream myfile("../output_L1L1/ubTest/cutEffects.txt", std::ios_base::out);
 ...
 eff->Draw("ap");
 ccr->Update();
 ccr->SaveAs("../output_L1L1/ubTest/cutEfficiencies.C");
 ccr->Close();
 ...
 llzs->Draw();
 cczs->Update();
 cczs->SaveAs("../output_L1L1/ubTest/zVertexAccidentals.C");
 cczs->Close();
