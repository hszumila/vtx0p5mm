...
int main(){
  ...
  TFile *fout = new TFile("../output_L1L1/ub/cutPlots.root","RECREATE");
  ...
  ltime->Draw();
  ctime->Update();
  ctime->SaveAs("../output_L1L1/ub/tdiff_cuts.C");
  ctime->Close();
  ...
  lzvtx->Draw();
  czvtx->Update();
  czvtx->SaveAs("../output_L1L1/ub/zvtx_cuts.C");
  czvtx->Close();
  ...
  lmass->Draw();
  cmass->Update();
  cmass->SaveAs("../output_L1L1/ub/mass_cuts.C");
  cmass->Close();
 ...
 h_zvm->Draw("colz");
 vertex->Update();
 vertex->SaveAs("../output_L1L1/ub/vertex_cuts.C");
 vertex->Close();
 ...
 llz->Draw();
 ccz->Update();
 ccz->SaveAs("../output_L1L1/ub/zVertexCutEffects.C");
 ccz->Close();

 

 //Plot the change in number of events from cuts in the peak versus the background
 std::fstream myfile("../output_L1L1/ub/cutEffects.txt", std::ios_base::out);
 ...
 eff->Draw("ap");
 ccr->Update();
 ccr->SaveAs("../output_L1L1/ub/cutEfficiencies.C");
 ccr->Close();
 ...
 llzs->Draw();
 cczs->Update();
 cczs->SaveAs("../output_L1L1/ub/zVertexAccidentals.C");
 cczs->Close();
